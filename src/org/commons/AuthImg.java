...
public class AuthImg {
	public static final String SESSION_IMGSTR = "SESSION_IMGSTR";
	// 定义图形验证码中绘制字符的字体
	...
	// 验证码类型 0数字组合，1数字与小写字母组合，2数字、小写字母与大写字母组合
	...
	// 输出图片流
	public static void buildImageCode() throws IOException {
		...
		//CT.setSessionAttr(true, SESSION_IMGSTR, sRand);
		g.dispose();
		ImageIO.write(image, "JPEG", CT.getRespOutputStream());
	}

	// 定义获取随机字符串方法
	private static String getRandomChar() {
		// 生成一个0、1、2的随机数字
		int rand = (int) Math.round(Math.random() * CHAR_TYPE);
		long itmp = 0;
		char ctmp = '\u0000';
		switch (rand) {
		// 生成大写字母
